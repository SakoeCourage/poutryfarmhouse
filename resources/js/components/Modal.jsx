...
export default function Modal(props) {
    return (<div id="modal"   className={`fixed bg-black/20 isolate backdrop-blur-[1px] flex items-center justify-center z-[60]   overflow-hidden inset-0 h-screen   ${props.className}`}>
    <motion.div 
    ...
    className="md:relative  static inset-0 w-max   z-[60]  md:inset-auto md:min-h-max  md:mx-auto ">
            ...
            <SimpleBar className=" relative  bg-white rounded-lg shadow-lg border-gray-200 border  w-screen h-screen md:h-max md:max-h-[32rem]   md:max-w-2xl   ">
              <div className='flex items-center justify-between sticky top-0 w-full bg-white z-50 rounded-t-lg px-5'>
                ...
                {props.title && <h3 className="text-lg font-semibold text-gray-800 lowercase lg:text-lg capitalize ">
                    {props.title ?? 'title'}
                </h3>}
            </div>
            </div>
