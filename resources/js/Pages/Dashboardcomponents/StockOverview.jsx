...
export default function StockOverview() {
        const {feed_and_stock} = usePage().props
          ...
          <nav className="text-slate-800 font-bold text-lg" >Stock data </nav>
            ...
            <nav className='flex relative flex-col isolate gap-2 p-2 w-full h-full shadow-md rounded-lg  aspect-[4/2]  '>
                ...
                <nav className='mt-auto px-5 text-white'>
                    ...
                    <nav className='font-semibold text-2xl'>
                        {formatcurrency(feed_and_stock.product_stock.value_in_stock)}
                    </nav>
                </nav>

                <nav className='mt-auto bg-indigo-300/90 border border-indigo-200 rounded-md p-5 text-sm'>
                    <nav className='flex items-center gap-2'><FontAwesomeIcon className='text-indigo-100' icon='check-double' /><span>{formatnumber(feed_and_stock.product_stock.number_of_products)} products found</span></nav>
                    <nav className='flex items-center gap-2'><FontAwesomeIcon className='text-indigo-100' icon='check-double' /><span>{formatnumber(feed_and_stock.product_stock.quantity_of_products)} products quantity in stock</span></nav>
                </nav>
            ...
            <nav className='flex relative flex-col isolate gap-2 p-2 w-full h-full shadow-md rounded-lg  aspect-[4/2]  '>
                ...
                <nav className='mt-auto px-5 text-white'>
                    <nav className='text-sm mb-2'>approximated value in stock</nav>
                    <nav className='font-semibold text-2xl'>
                        {formatcurrency(feed_and_stock.feed_stock.value_in_stock)}
                    </nav>
                </nav>

                <nav className='mt-auto bg-blue-300/90 border border-blue-200 rounded-md p-5 text-sm'>
                    <nav className='flex items-center gap-2'><FontAwesomeIcon className='text-blue-100' icon='check-double' /><span>{formatnumber(feed_and_stock.feed_stock.number_of_feeds)} feeds types </span></nav>
                    <nav className='flex items-center gap-2'><FontAwesomeIcon className='text-blue-100' icon='check-double' /><span>{new Intl.NumberFormat('en-GB', {
                        ...
                        unit: 'kilogram',
                        unitDisplay: 'short',
                    }).format(feed_and_stock.feed_stock.quantity_of_feeds_left)} of all feeds left</span></nav>
                </nav>
