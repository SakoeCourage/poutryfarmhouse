...
export default function Controlproducts(props) {
    ...
    let checkIfInCrate = (index) => {
        return (Boolean(products.find(pro => pro.id == data[index].product_id)?.in_crates))
    }
    let getCollectionTypes = (index) => {
        return (products.find(pro => pro.id == data[index].product_id)?.collection_type)
    }

    useEffect(() => {
        data.map((item, i) => {
            if (item.product_id) {
                let units_per_crate = products.find(pro => pro.id == data[i].product_id)?.definitions[0]?.units_per_crate
            ...
            <fieldset className='border py-4 px-2'>
                ...
                <div className='flex flex-col gap-3 ' >
                    {data.map((item, i) => <div className={`flex items-center gap-1  salelist `} data-index={i + 1} key={i}>
                        ...
                        <nav className='text-gray-500 basis-5/12 flex items-center gap-2'>
                            {checkIfInCrate(i) && <Custominput className="grow" error={props.errors[`products.${i}.crates`]} value={item.crates} placeholder={`enter ${getCollectionTypes(i)+'(s)'}`} getValue={(value) => handleValueChange(i, "crates", value)} />}
                            <Custominput className=" grow " error={props.errors[`products.${i}.units`]} value={item.units} placeholder="enter units" getValue={(value) => handleValueChange(i, "units", value)} />
                        </nav>
