...
export default function Addtofeedform(props) {
    const [errors,setErrors] = useState({});
    const [processing,setProcessing] = useState(false)
    const [data,setData] = useState({
      'quantity':null,
      'description': '',
      'feed_id':props.feedDetails?.id
    })
    const [currentRoute,setCurrentRoute] = useState('/stock/feedstock/add')
    const handleSubmit= (e) =>{
      e.preventDefault()
      if(quantityAfterUpdate >= 0){
      setProcessing(true)
      Api.post(currentRoute,data).then(res=>{
        props.handleOnSucess(props.feedDetails.id)
      }).catch((err)=>{
        console.log(err)
    ...
    const quantityAfterUpdate = useMemo(() =>{
      if(currentRoute == '/stock/feedstock/add'){
        return  Number(props.feedDetails?.quantity) + Number(data.quantity) 
      }else if(currentRoute == '/stock/feedstock/remove'){
        return Number(props.feedDetails?.quantity) - Number(data.quantity) 
      }
  ...
  return (
    ...
    <div className='flex items-center mb-5 bg-gray-100 sticky top-0 mx-5 md:mx-10 '>
      <button
      onClick={()=>setCurrentRoute('/stock/feedstock/add')}
      className={`w-full p-2 rounded-md text-sm md:text-base  ${currentRoute =='/stock/feedstock/add' && 'bg-indigo-400 text-white' }`}><span>Increase Stock Quantity</span></button>
      <button 
      onClick={()=>setCurrentRoute('/stock/feedstock/remove')}
      className={`w-full p-2 rounded-md text-sm md:text-base ${currentRoute =='/stock/feedstock/remove' && 'bg-red-300 text-white' }`} ><span>Decrease Stock Quantity </span></button>
  </div>
